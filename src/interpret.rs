...
Each file starts with a header (path, type, size, score), then its context. A finding is announced on its own comment line — `# SEV LINE:COL desc` or `// SEV LINE:COL desc` — placed immediately BEFORE the source line it describes (SEV is H>S>N>B = hostile/suspicious/notable/baseline; LINE:COL is omitted for whole-file findings). The line(s) that follow that annotation are the file's own source, shown unaltered. Binary regions render as printable text with C-style escapes (`\\xNN`, `\\0`, `\\n`) for non-printable bytes.\n\
