#!/bin/sh
# freebsd-rcd.sh - Shared helpers for the FreeBSD litmus worker rc.d service.
#
...
# worker count. Usage: litmus_worker_args <url> [workers]
litmus_worker_args() {
	_lwa_url="$1"
	_lwa_workers="$2"
...
# Usage: litmus_rcd_script <binary_path> <worker_args>
#
...
# core. rc's `litmus_worker_enable=YES` brings it back across reboots, so the
# only way the worker stays down is an explicit `service litmus-worker stop`.
litmus_rcd_script() {
	_lrs_bin="$1"
	_lrs_worker_args="$2"
	cat <<EOF
...
#           pidfile; \`service litmus-worker stop\` signals it to tear the
#           whole tree down.
command_args="-c -f -r -R 5 -P \${pidfile} -o \${litmus_worker_logfile} -u litmus /usr/bin/env MALLOC_CONF=\${malloc_conf} $_lrs_bin $_lrs_worker_args"
