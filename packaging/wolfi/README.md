...
The local build needs both repos checked out next to scan:
...
2. **Stages cleave + filefacts + scan** into `out/wolfi/source/` with
...
5. **Substitutes `cd scan`** for the `# LOCAL_BUILD_CD_HERE` marker
   ...
   `/home/build` contains both `cleave/` and `scan/` subdirs.
...
1. **filefacts path dep in cleave** — see `cleave/packaging/wolfi/README.md`.
   Until cleave resolves this, neither cleave nor scan can land in
   wolfi-dev/os.
2. **scan's git dep on cleave** — Wolfi prefers package-to-package
   deps. Once cleave is in wolfi-dev/os, scan's Cargo.toml should
...
- **`[patch]` over Cargo.toml dep rewriting** — scan's cleave
