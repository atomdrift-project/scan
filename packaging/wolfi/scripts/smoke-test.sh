...
IMAGE_TAR="$OUT_DIR/scan.tar"
IMAGE_REF="scan:smoke"
...
os=$(uname -s)
case "$os" in
  Darwin) NERDCTL="limactl shell --workdir / $VM_NAME nerdctl" ; IMAGE_IN_VM="/work/out/scan.tar" ;;
  Linux)
...
if [ "$rc" -ge 3 ]; then
  echo "  FAIL  scan returned error exit $rc; output: $(head -20 /tmp/scan-smoke.out)" >&2
  fail=1
else
