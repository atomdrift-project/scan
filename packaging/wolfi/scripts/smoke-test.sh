#!/bin/sh
# Load the apko-built litmus image and assert it works end-to-end:
#  1. litmus --version reports the expected version
#  2. litmus --help runs cleanly
#  3. A scan against a known-clean binary succeeds with the bundled models
#  4. Same scan with bogus LITMUS_MODELS_DIR errors out (proves models
#     are actually being consulted, not silently skipped)
#
# Litmus exit codes (per project README): 0=clean, 1=hostile,
...
IMAGE_TAR="$OUT_DIR/litmus.tar"
IMAGE_REF="litmus:smoke"
...
os=$(uname -s)
case "$os" in
  Darwin) NERDCTL="limactl shell --workdir / $VM_NAME nerdctl" ; IMAGE_IN_VM="/work/out/litmus.tar" ;;
  Linux)
...
echo "==> 1/4 litmus --version reports $EXPECTED_VERSION"
out=$($NERDCTL run --rm "$IMAGE_REF" --version 2>&1 || true)
echo "$out" | grep -q "$EXPECTED_VERSION"
report $? "saw version: $out"

echo "==> 2/4 litmus --help runs"
$NERDCTL run --rm "$IMAGE_REF" --help >/dev/null 2>&1
report $? "help exits 0"

echo "==> 3/4 scan with bundled models succeeds (any verdict, not error)"
# `|| rc=$?` so set -e doesn't abort on litmus's 1/2 verdict exits.
rc=0
$NERDCTL run --rm "$IMAGE_REF" /usr/bin/cleave >/tmp/litmus-smoke.out 2>&1 || rc=$?
# 0=clean, 1=hostile, 2=suspicious all mean the scan worked. 3+ = errors.
if [ "$rc" -ge 3 ]; then
  echo "  FAIL  scan returned error exit $rc; output: $(head -20 /tmp/litmus-smoke.out)" >&2
  fail=1
else
  report 0 "scan of /usr/bin/cleave returned verdict exit=$rc"
fi

echo "==> 4/4 scan with bogus LITMUS_MODELS_DIR errors out"
out=$($NERDCTL run --rm \
  --env LITMUS_MODELS_DIR=/nonexistent-models \
  "$IMAGE_REF" /usr/bin/cleave 2>&1 || true)
if echo "$out" | grep -qi 'LITMUS_MODELS_DIR\|models\|nonexistent'; then
