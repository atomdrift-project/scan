#!/bin/sh
# Idempotently ensure the scan-wolfi Lima VM is up and ready to build.
...
CACHE_DIR="${XDG_CACHE_HOME:-$HOME/.cache}/scan-wolfi"
...
# bundling the cleave apk into scan's image.
[ -d "$SRC_DIR/cleave" ] || {
  echo "error: sibling 'cleave' repo not found at $SRC_DIR/cleave" >&2
  echo "       scan's wolfi build needs cleave checked out alongside it." >&2
  exit 1
}
...
case "$os" in
  Darwin)
    ...
    if ! limactl list --format '{{.Name}}' 2>/dev/null | grep -qx "$VM_NAME"; then
      ...
      rendered=$(mktemp -t scan-wolfi-lima.XXXXXX)
