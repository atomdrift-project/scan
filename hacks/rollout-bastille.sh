...
doas bastille pkg "$BUILD" install -y rust sccache git onnxruntime
...
doas bastille cmd "$BUILD" su -l litmus -c "cd ~/litmus && RUSTC_WRAPPER=sccache ORT_STRATEGY=system ORT_LIB_DIR=/usr/local/lib make tarball"
